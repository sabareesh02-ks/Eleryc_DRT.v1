...
def analyze_raw_data():
    """
    Analyze uploaded CSV file and return statistics
    """
    try:
        ...
        for col in df.columns:
            ...
            if pd.api.types.is_numeric_dtype(df[col]):
                stats['min'] = float(df[col].min()) if pd.notna(df[col].min()) else None
                stats['max'] = float(df[col].max()) if pd.notna(df[col].max()) else None
                stats['mean'] = float(df[col].mean()) if pd.notna(df[col].mean()) else None
                stats['std'] = float(df[col].std()) if pd.notna(df[col].std()) else None
            else:
